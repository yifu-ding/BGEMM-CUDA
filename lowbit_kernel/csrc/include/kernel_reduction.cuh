...
__global__ void SplitK_Reduction(uint32_t* C, float* Reduction_Workspace, size_t M_Global, size_t N_Global, int Split_K)
{
    uint32_t*  WARP_GPTR_C      = C                    + REDUCTION_ELEMENT_PER_THREADBLOCK * blockIdx.x;
    float* WARP_GPTR_R      = Reduction_Workspace  + REDUCTION_ELEMENT_PER_THREADBLOCK * blockIdx.x;
    uint32_t*  THREAD_GPTR_C    = WARP_GPTR_C          + threadIdx.x * UINT32_PER_128BIT;    
    float* THREAD_GPTR_R    = WARP_GPTR_R          + threadIdx.x * UINT32_PER_128BIT;
    // Initializing Thread-Local Results
    uint32_t Results[UINT32_PER_128BIT];
    #pragma unroll
    for (int i = 0; i < UINT32_PER_128BIT; i++)       Results[i] = 0.0f;
    // Reduction
    for (int i = 0; i < Split_K; i++) {
        #pragma unroll
        for (int j = 0; j < UINT32_PER_128BIT; j++)   Results[j] += THREAD_GPTR_R[j];
        THREAD_GPTR_R += M_Global * N_Global;
    }
    // Writing to global memory
    // #pragma unroll
    for (int i = 0; i < UINT32_PER_128BIT; i++)       THREAD_GPTR_C[i] = Results[i];
}
