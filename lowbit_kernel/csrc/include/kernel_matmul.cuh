                                  ...
                                  int Split_K) 
{
  ...
  // // 2+4 weight split
  // Dynamic shared memory for FP16 A tiles， 128 Bytes aligned
  extern __shared__ __align__(128) uint32_t smem_weight[];   // 0x0200
  // extern __shared__ __align__(128) half smem_act[];   // 0x0200
  uint32_t (*smem_act)[WARP_K_BIN/32+PADDING_SHARED_MEM_FOR_B_1] = reinterpret_cast<uint32_t (*)[WARP_K_BIN/32+PADDING_SHARED_MEM_FOR_B_1]> ( smem_weight + (SMEM_SIZE_TILE)/2 ); // Dynamic shared memory for FP16 Act tiles  // 0x7fffd5002400
  ...
  // // StartPTR for each WARP.
  const uint4* WARP_StartGPTR_W  = TB_StartGPTR_W + WARP_i * NumBlock_K * NUM_INT4_PER_UNIT_BIN;   // 0x7fffa0c00000
  // // StartPTR for each WARP, considering SplitK
  const size_t     WARP_Start_UnitID_K = StartBlockID_K;  // unsigned long = size_t = 32bits
  WARP_StartGPTR_W  += WARP_Start_UnitID_K; 
  // // Copying A tile from Global to Shared, using double-buffer //////////////////////////////////////////////////////////
  // // StartSPTR for each ThreadBlock
  uint32_t* Weight_SPTR = reinterpret_cast<uint32_t*>(smem_weight);   // 0x7fffd5000400
  // // StartSPTR for each WARP
  Weight_SPTR += warpId * SMEM_SIZE_IN_BYTES_PER_WARP_BIN/4;
  // // Pre-fetch of A tile
  for(int i=0; i<PIPELINE_LEVEL_GMEM-1; i++) {
...
// #endif
  ...
  uint32_t (*smem_CFrag) [TilingConfig::TILE_M_BIN+PADDING_SHARED_MEM_FOR_C_0] =
        reinterpret_cast <uint32_t (*)[TilingConfig::TILE_M_BIN+PADDING_SHARED_MEM_FOR_C_0]> (smem_weight);
  StoreToSharedMemoryFromRegister<TilingConfig>(smem_CFrag, c, NumIterB);
  __syncthreads();
  // Now that shared memory contains all the D tiles, stream them to global memory.
  OutputDataType* BlockGlobalPTR = C + BatchID*(M_Global*N_Global) + Tile_Start_M + Tile_Start_N*M_Global;
  for(size_t i=warpId; i<NumColumnToCopy; i+=TilingConfig::BLOCK_WARPS)    // i-th column
