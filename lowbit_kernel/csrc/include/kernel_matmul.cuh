...
__global__ void FP16_GEMM_Kernel(const half* Weight, // const half* S_w, const half*  S_a, 
                                  ...
                                  int Split_K) 
{
  ...
  half (*smem_act)[WARP_K_BIN+PADDING_SHARED_MEM_FOR_B_1] = reinterpret_cast<half (*)[WARP_K_BIN+PADDING_SHARED_MEM_FOR_B_1]> ( smem_weight_half + WEIGHT_PER_UNIT_BIN/32*3); // Dynamic shared memory for FP16 Act tiles  // 0x7fffd5002400
  ...
  const half* TB_StartGPTR_W = Weight + (y*TilingConfig::BLOCK_ROW_WARPS)*NumBlock_K*(HALF_WEIGHT_PER_UNIT/32/4); // /4 是用来抵消BLOCK_ROW_WARPS
  ...
  const half* WARP_StartGPTR_W  = TB_StartGPTR_W + WARP_i * NumBlock_K * (HALF_WEIGHT_PER_UNIT/4/32);   // 0x7fffa0c00000
  ...
  Weight_SPTR += (warpId * HALF_WEIGHT_PER_UNIT/4/32);
  // // Pre-fetch of A tile
  for(int i=0; i<PIPELINE_LEVEL_GMEM-1; i++) { 
    ...
    CopyFromGlobalToShared_A<TilingConfig::TILE_N_BIN, TilingConfig::BLOCK_WARPS>(smem_act+(i*TilingConfig::TILE_K_BIN), TB_StartGPTR_A, K_Global, NumColumnToCopy);
  ...
  uint32_t a  [NumRegSets_w * PIPELINE_LEVEL_SMEM][8];      // double/Trible buffer is used // Registers to store decompressed FP6
  uint32_t b  [NumRegSets_a * PIPELINE_LEVEL_SMEM][4];      // double/Triple buffer is used // Register to store FP16 Act matrix (a slice)
#endif
  float c[NumRegSets_w * NumRegSets_a][REG_PER_THREAD_C_TENSOR_16_16]; // REG_PER_THREAD_C = 2 // 
  ...
  half* Weight_SPTR_Start = reinterpret_cast<half*>(smem_weight_half);
  initialize_mma_slice_fp<TilingConfig>(a, b, Weight_SPTR_Start, smem_act, NumIterB);
#endif
  // The outer loop. /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  #pragma unroll(1)
...
// #endif
    half* __restrict__ read_SPTR_W  = Weight_SPTR_Start + ((tile_id_k+0)   % PIPELINE_LEVEL_GMEM) * WEIGHT_PER_UNIT_BIN/32; // 1024 (1)*4: 4 WARPs; (2)/4: int*+1 = char*+16
    // read_SPTR_W = 0x7fffd5000400
#ifdef PIPELINE_LEVEL_SMEM
    half* __restrict__ read2_SPTR_W  = Weight_SPTR_Start + ((tile_id_k+1)  % PIPELINE_LEVEL_GMEM) * WEIGHT_PER_UNIT_BIN/32;
    // read2_SPTR_W = 0x7fffd5000c00
#endif
    ...
    half* __restrict__ write_SPTR_W = Weight_SPTR + ((tile_id_k+(PIPELINE_LEVEL_GMEM-1))  % PIPELINE_LEVEL_GMEM) * WEIGHT_PER_UNIT_BIN/32; // 1024 (1)*4: 4 WARPs; (2)/4: int*+1 = char*+16
...
// #endif
    ...
    // copying Act tile from GlobalMemory to SharedMemory
	  CopyFromGlobalToShared_A<TilingConfig::TILE_N_BIN, TilingConfig::BLOCK_WARPS>(write_SPTR_A, TB_StartGPTR_A, K_Global, NumColumnToCopy, GlobalCopy);

    cp_async_group_commit();
  #ifdef PIPELINE_LEVEL_SMEM
    uint32_t (*b_read )[4] = b;  // 4*1
    uint32_t (*b_write)[4] = b; 
    if (tile_id_k%2==1) {b_read += NumRegSets_a; } else {b_write += NumRegSets_a;}
    core_mma_slice_fp<TilingConfig>(c, a, b_read, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  1, NumIterB); // read_SPTR_W, read_SPTR_Frag2 are different for each WARP; read_SPTR is shared among WARPs
    core_mma_slice_fp<TilingConfig>(c, a, b_read, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  2, NumIterB);
    core_mma_slice_fp<TilingConfig>(c, a, b_read, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  3, NumIterB);
    // Barriers and Synchronizations
    cp_async_wait_group<PIPELINE_LEVEL_GMEM-2>();
    __syncthreads();
    core_mma_slice_fp<TilingConfig>(c, a, b_read, read2_SPTR_W, read2_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  0, NumIterB);
  ...
  float (*smem_CFrag) [TilingConfig::TILE_M+PADDING_SHARED_MEM_FOR_C_4] =
        reinterpret_cast <float (*)[TilingConfig::TILE_M+PADDING_SHARED_MEM_FOR_C_4]> (smem_weight_half);
...
template<typename TilingConfig, typename OutputDataType>
__global__ void PACK_AttV_Kernel(const half* Weight, const half* S_w, const half*  S_a, 
                                  const half *Act,  // 0x7fffa0c22200
                                  // const half *Act,
                                  OutputDataType* C,
                                  const size_t M_Global, const size_t N_Global, const size_t K_Global,
                                  ...
                                  int INSTR) 
{
  ...
  int NumSlices = 4;
  int NumIterB = WARP_N_BIN/NumSlices/8; // =32/4/8=1
  initialize_mma_slice_attv<TilingConfig>(a, b, Weight_SPTR, smem_act, NumIterB);
#endif
  // The outer loop. /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  #pragma unroll(1)
  for (size_t tile_id_k = 0; tile_id_k < NumIter; tile_id_k++)
...
// #endif
    ...
    uint32_t (*a_write)[1] = a; 
    if (tile_id_k%2==1) {a_read += NumRegSets_w; } else {a_write += NumRegSets_w;}
    core_mma_slice_attv<TilingConfig>(c, a_read, b, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  1, NumIterB, INSTR); // read_SPTR_W, read_SPTR_Frag2 are different for each WARP; read_SPTR is shared among WARPs
    core_mma_slice_attv<TilingConfig>(c, a_read, b, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  2, NumIterB, INSTR);
    core_mma_slice_attv<TilingConfig>(c, a_read, b, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  3, NumIterB, INSTR);
    // Barriers and Synchronizations
    cp_async_wait_group<PIPELINE_LEVEL_GMEM-2>();
    __syncthreads();
    core_mma_slice_attv<TilingConfig>(c, a_read, b, read2_SPTR_W, read2_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  0, NumIterB, INSTR);
    PackFromSharedToRegister_BinaryW<1, 1>   (a_write, read2_SPTR_W, 0);
    // Updating global PTRs
    WARP_StartGPTR_W +=  TilingConfig::TILE_K_BIN; // 128 half / block
    TB_StartGPTR_A += TilingConfig::TILE_K_BIN;    // 128 half
...
template<typename TilingConfig, typename OutputDataType>
__global__ void PACK_QK_Kernel(const half* Weight, const half* S_w, const half*  S_a, 
                                  const half *Act,  // 0x7fffa0c22200
                                  // const half *Act,
                                  OutputDataType* C,
                                  const size_t M_Global, const size_t N_Global, const size_t K_Global,
                                  ...
                                  int INSTR) 
{
  ...
  // constexpr int NumRegSets_a = (TilingConfig::WARP_COL_MMA_TENSORS==1) ? 1 : TilingConfig::WARP_COL_MMA_TENSORS/2;    // 1 set = 4 registers, containing a 16*16 MMA block
#ifdef PIPELINE_LEVEL_SMEM
  uint32_t a  [NumRegSets_w * PIPELINE_LEVEL_SMEM][2];      // double/Trible buffer is used // Registers to store decompressed FP6
  uint32_t b  [NumRegSets_a * PIPELINE_LEVEL_SMEM][2];      // double/Triple buffer is used // Register to store FP16 Act matrix (a slice)
#endif
  int32_t c[NumRegSets_w * NumRegSets_a][REG_PER_THREAD_C_TENSOR_16_16]; // REG_PER_THREAD_C = 2 // 
  for(int i=0; i<NumRegSets_w * NumRegSets_a; i++) 
  ...
  int NumSlices = 4;
  int NumIterB = WARP_N_BIN/NumSlices/8; // =32/4/8=1
  initialize_mma_slice_qk<TilingConfig>(a, b, Weight_SPTR, smem_act, NumIterB);
#endif
  // The outer loop. /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  #pragma unroll(1)
  for (size_t tile_id_k = 0; tile_id_k < NumIter; tile_id_k++)
...
// #endif
    ...
    cp_async_group_commit();
  #ifdef PIPELINE_LEVEL_SMEM
    uint32_t (*a_read )[2] = a;  // 4*1
    uint32_t (*a_write)[2] = a; 
    if (tile_id_k%2==1) {a_read += NumRegSets_w; } else {a_write += NumRegSets_w;}
    core_mma_slice_qk<TilingConfig>(c, a_read, b, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  1, NumIterB, INSTR); // read_SPTR_W, read_SPTR_Frag2 are different for each WARP; read_SPTR is shared among WARPs
    core_mma_slice_qk<TilingConfig>(c, a_read, b, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  2, NumIterB, INSTR);
    core_mma_slice_qk<TilingConfig>(c, a_read, b, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  3, NumIterB, INSTR);
    // Barriers and Synchronizations
    cp_async_wait_group<PIPELINE_LEVEL_GMEM-2>();
    __syncthreads();
    core_mma_slice_qk<TilingConfig>(c, a_read, b, read2_SPTR_W, read2_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  0, NumIterB, INSTR);
    PackFromSharedToRegister_BinaryAct_w2a3<2>   (a_write, read2_SPTR_W, 0);
    // Updating global PTRs
    WARP_StartGPTR_W += TilingConfig::TILE_K_BIN; // 128 half / block
    TB_StartGPTR_A += TilingConfig::TILE_K_BIN;    // 128 half
  #else
...
template<typename TilingConfig, typename OutputDataType>
__global__ void PACK_W2A3_Kernel(const uint32_t* Weight, const half* S_w, const half*  S_a, 
                                  const half *Act,  // 0x7fffa0c22200
                                  // const half *Act,
                                  OutputDataType* C,
                                  const size_t M_Global, const size_t N_Global, const size_t K_Global,
                                  ...
                                  int INSTR) 
{
  ...
  initialize_mma_slice_binpack_w2a3<TilingConfig>(a, b, Weight_SPTR_Start, smem_act, NumIterB);
#endif
  // The outer loop. /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  #pragma unroll(1)
  for (size_t tile_id_k = 0; tile_id_k < NumIter; tile_id_k++)
...
// #endif
    ...
    if (tile_id_k%2==1) {b_read += NumRegSets_a; } else {b_write += NumRegSets_a;}
    core_mma_slice_binpack_w2a3<TilingConfig>(c, a, b_read, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  1, NumIterB, INSTR); // read_SPTR_W, read_SPTR_Frag2 are different for each WARP; read_SPTR is shared among WARPs
    core_mma_slice_binpack_w2a3<TilingConfig>(c, a, b_read, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  2, NumIterB, INSTR);
    core_mma_slice_binpack_w2a3<TilingConfig>(c, a, b_read, read_SPTR_W, read_SPTR_A, Scales_RPTR_w, Scales_RPTR_a,  3, NumIterB, INSTR);
    // Barriers and Synchronizations
    cp_async_wait_group<PIPELINE_LEVEL_GMEM-2>();
    __syncthreads();
    ...
    // CopyFromSharedToRegister_BinaryW<1, 1>   (a_write, read2_SPTR_W, 0);
    PackFromSharedToRegister_BinaryAct_w2a3<2>  (b_write, read2_SPTR_A, NumIterB);
